...
def main(hyperparameter):
    ...
    SELECTED_FEATURES_PATH = "feature_table_1C.csv"
    ...
    acc_list = []
    num_folds_list = []
    for NUM_VALIDATION_FOLDS in range(2,len(y)-10):
        accscores = []
        for train, test in get_folds(X, y, groups=groups,
            ...
            val_loss_list = []
            loss_list = []
            model, optimizer = feed_forward_model_builder.init_system(hyperparameter, n_features,n_classes)
            ...
            if n_classes == 2:
                ...
                for test, pred in zip(test_labels, y_pred):
        ...
        print(f"Accuracy Ave = {np.mean(accscores)}")
        ...
        """
        std_acc_list.append(np.std(accscores))
        acc_list.append(np.mean(accscores))
        num_folds_list.append(NUM_VALIDATION_FOLDS)

    return [acc_list,num_folds_list]
